...
| `wrapSelection(pre, post, anchor, focus)` | Wraps the current selection (if `anchor` and `focus` are `null` or omitted) or a specific selection (if `anchor` and `focus` are given) in the strings `pre` and `post`. For example, `wrapSelection('[', '](https://www.github.com)')` will wrap the selection with a link to GitHub. Fires a `change` event (and updates the linked textarea, if any). Does nothing if the selection spans more than one line.                                                                                                             |
...
| `removeEventListener(type, listener)`     | Removes an [event listener](#event-listeners) previously added with `addEventListener()`. The `listener` has to be the same function object that was passed to `addEventListener()`.                                                                                                                                                                                                                                                                                                                                    |
| `destroy()`                               | Tears the editor down: removes every event listener it registered (including the one on `document`, which would otherwise keep the editor alive for the lifetime of the page), removes the editor element if TinyMDE created it, and shows the linked textarea again. An editor element you passed in yourself is left in the DOM, emptied and restored to the state it was in before. Safe to call more than once; the editor must not be used afterwards. Does **not** destroy a linked CommandBar—see [below](#commandbar-methods). |
...
| Method            | Description                                                                                                                                                                                                                                                                                        |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `setEditor(editor)` | Links the command bar to an editor, detaching it from the editor it was previously linked to (if any).                                                                                                                                                                                            |
| `destroy()`       | Tears the command bar down: removes every event listener it registered (including the one on `document`), detaches from its editor, and removes the command bar element from the DOM. Safe to call more than once; the command bar must not be used afterwards.                                      |

### Tearing down an editor

If you create and destroy editors repeatedly—for example in a component-based framework such as React, Vue or Svelte, where the component holding the editor is mounted and unmounted as the user navigates—call `destroy()` when you are done with an instance. Both `Editor` and `CommandBar` register listeners on `document` that live as long as the page does otherwise.

Editor and CommandBar are destroyed independently, so destroy both:
